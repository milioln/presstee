...
export function analyse(): void {
  ...
  if (S.vector) {
    ...
    const im = new Image();
    im.onload = () => {
      S.natW = im.naturalWidth || 0;
      S.natH = im.naturalHeight || 0;
      S.colors = null;
      S.dom = null;
      S.colorSwatches = null;
      paintDiag();
      paintTechs();
      paintRecap();
    };
  ...
  im.onload = () => {
    ...
    try {
      ...
      S.colorSwatches = sig.slice(0, 12).map((e) => toHex(e.r / e.c, e.g / e.c, e.b / e.c));
      const t = sig[0];
